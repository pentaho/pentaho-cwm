...
/**
 * SoftwareDeployment package interface.
 ...
 * The Software Deployment package contains classes to record how the softw
 * are in a data warehouse is used.
 * A software package is represented as a SoftwareSystem object, which is a
 *  subtype of Subsystem. A SoftwareSystem may reference one or more TypeSystems that def
 * ine the datatypes supported by the SoftwareSystem. The mappings between
 *  datatypes in different TypeSystems may be recorded as TypeMappings, as de
 * scribed in the TypeMapping metamodel.
 * The separate components of a software package are each represented as Co
 * mponents that are either owned or imported by the SoftwareSystem. When a SoftwareSyste
 * m is installed, the deployment is recorded as a DeployedSoftwareSystem 
 * and a set of DeployedComponents.
 * A DeployedComponent represents the deployment of a specific Component on
 *  a specific computer. Dependencies between DeployedComponents on the same computer may
 *  be documented as Usage dependencies between them.
 * Individual computers are represented as Machine objects, located at a Si
 * te. A Site represents a geographical location, which may be recorded at any relevant l
 * evel of granularity, e.g. a country, a building, or a room in a buildin
 * g. Hierarchical links between Sites at different levels of granularity may
 *  be documented.
 * A DataManager is a DeployedComponent such as a DBMS or file management s
 * ystem that provides access to data. It may be associated with one or more data Package
 * s identifying the Schema, Relational Catalog, Files or other data conta
 * iners that it provides access to.
 * A DataProvider is a DeployedComponent that acts as a client to provide a
 * ccess to data held by a DataManager. For example, an ODBC or JDBC client on a specific
 *  Machine would be represented as a DataProvider. A DataProvider may hav
 * e several ProviderConnections, each identifying a DataManager that may be 
 * accessed using the DataProvider.
 * If a DataProvider uses a name for a data Package that is different from 
 * the actual name used by the DataManager, a PackageUsage object can be added to record 
 * this. 
 * As a DataProvider is a subtype of DataManager, it is possible for a Data
 * Provider to access data from a DataManager which is actually a DataProvider acting as 
 * a client to yet another DataManager.
 * The model for the Software Deployment package is shown in three diagrams
 * . The first diagram shows the objects related to software deployment, while the second
 *  diagram displays the DataManager and DataProvider area of the model. T
 * he third diagram shows the inheritance structure for all the classes in th
 * e Software Deployment package.
 * OCL Representation of SoftwareDeployment Constraints
 * [C-8-1] A PackageUsage must have a single Package (or subtype of Package
 * ) as its supplier
 * context PackageUsage inv:
 * self.supplier->size=1 and
 * self.supplier->at(1).oclIsKindOf(Package)
 * [C-8-2] A ProviderConnection must not associate a DataProvider with itse
 * lf
 * context ProviderConnection inv:
 * self.dataManager <> self.dataProvider
 * [C-8-3] A Site must not have a containingSite reference that refers to i
 * tself.
 * context Site inv:
...
public interface SoftwareDeploymentPackage extends javax.jmi.reflect.RefPackage {
     ...
     */
    public org.pentaho.pms.cwm.pentaho.meta.softwaredeployment.CwmDeployedSoftwareSystemClass getCwmDeployedSoftwareSystem();
    /**
     ...
     */
    public org.pentaho.pms.cwm.pentaho.meta.softwaredeployment.DeployedSoftwareSystemComponents getDeployedSoftwareSystemComponents();
    /**
