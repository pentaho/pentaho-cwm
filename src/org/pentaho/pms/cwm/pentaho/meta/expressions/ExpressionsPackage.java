 ...
 * The CWM Expressions metamodel provides basic support for the definition of expression trees withi
 * n the CWM. The intent of the Expressions metamodel is to provide a place
 *  for other CWM packages (such as Transformation) and CWM compliant tools to 
 * record shared expressions in a common form that can be used for interchang
 * e and lineage tracking.
 * The expression concept in the CWM Foundation takes a functional view of expression trees, resulti
 * ng in the ability of relatively few expression types to represent a broa
 * d range of expressions. Every function or traditional mathematical operator 
 * that appears in an expression hierarchy is represented as a FeatureNode. F
 * or example, the arithmetic plus operation ?a + b? can be thought of as 
 * the function ?sum(a, b).? The semantics of a particular function or operat
 * ion are left to specific tool implementations and are not captured by the CWM
 * . 
 * The hierarchical nature of the CWM?s representation of expressions is achieved by the recursive n
 * ature of the OperationArgument association. This association allows the 
 * sub-hierarchies within an expression to be treated as actual parameters of t
 * heir parent nodes.
 * OCL Representation of Expressions Constraints
 * [C-5-1] A FeatureNode that has parameters other than the"this" parameter represents a Feature tha
 * t is also an Operation.
 ...
 *    self.feature.oclIsKindOf(Operation)
 * endif
 * [C-5-2] If the FeatureNode represents an instance-scope feature, the first argument is a "this" o
 * r "self" argument; that is, the object invoking the feature. The convent
 * ion is enforced by checking that the type of the first argument is the same 
 * as the type of the feature.
 * context FeatureNode inv:
 * self.feature.ownerScope = #instance implies
 * self.argument->first.type.allFeatures->includes(self.feature)
 * [C-5-3] If the FeatureNode represents a BehavioralFeature, the number of arguments must be equal 
 * to the number of the BehavioralFeature?s parameters, plus one for the ?t
 * his? parameter if the BehavioralFeature is of instance scope.
 * context FeatureNode inv:
