 ...
 * The Relational package describes data accessible through a relational interface 
 * such as a native RDBMS, ODBC, or JDBC. The Relational package is based 
 * on the [SQL] standard section concerning RDBMS catalogs.
 * The scope of the top level container, Catalog, is intended to cover all the tabl
 * es a user can use in a single statement. A catalog is also the unit whi
 * ch is managed by a data resource. A catalog contains schemas which them
 * selves contain tables. Tables are made of columns which have an associat
 * ed data type.
 * The Relational package uses constructs in the ObjectModel package to describe th
 * e object extensions added to SQL by the [SQL] standards. 
 * The Relational package also addresses the issues of indexing, primary keys and f
 * oreign keys by extending the corresponding concepts from the Foundation
 *  packages.
