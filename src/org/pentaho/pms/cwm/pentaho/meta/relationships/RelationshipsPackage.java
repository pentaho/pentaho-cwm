...
/**
 * Relationships package interface.
 * The Relationships metamodel depends on the following package:
 *     org.omg::CWM::ObjectModel::Core
 * The Relationships metamodel collects together classes and associations that describe  the relationsh
 * ips between object within a CWM information store. The Relationships me
 * tamodel describes to types of relationships: association and generalizati
 * on.
 * Association relationships record linkages between model elements. These linkages may represent simpl
 * e linkages between model elements or aggregation ("is part of") relatio
 * nships between model elements; aggregation relationships come in two form
 * s -- shared and composite. Associations have two or more named ends that
 *  link them to instances of the classes connected by the association.
 * Generalization relationships record arrangements of model elements into type hierarchies in a parent
 * /child (or "is type of") fashion. Child types are said to "specialize",
 *  "subclass" or "subtype" their parental  types, represent a subset of par
 * ental instances that fulfill the definition of the child type, and inher
 * it the structural features
 * (Attributes, AssociationEnd) and behavioral features (Operations, Methods) of their parents. Parenta
 * l types are said to "generalize" their child types or to be "superclass
 * es" or "supertypes" of their children.
 * CWM generalization hierarchies support multiple inheritance; that is, child types may have more than
 *  one parental type and inherit the union of the features of all their p
 * arental types. Although called "hierarchies", multiple inheritance actual
 * ly represents a directed acyclic graph of parental and child types.
 ...
 * [C-5-4] If an Association has three or more AssociationEnds, then no AssociationEnd may be an aggreg
 * ation or composition.
 * context Association inv:
 * self.allConnections->size >=3 implies
 * self.allConnections->forall(aggregation = #ak_none)
 * [C-5-5] The connected Classifiers of the AssociationEnds should be included in the Namespace of the 
 * Association, or be Classifiers with public visibility in other Namespac
 * es to which the Association has access.
 * context Association inv:
 ...
 * [C-5-7] The Classifier of an AssociationEnd cannot be an Interface or a DataType if the association 
 * is navigable away from that end.
 * context AssociationEnd inv:
 ...
 * [C-5-9] An AssociationEnd with composite or shared aggregation semantics must be owned by an Associa
 * tion.
 * context AssociationEnd inv:
