 ...
 * The Behavioral metamodel collects together classes and associations that describe the behavior o
 * f CWM types and provides a foundation for recording the invocations of de
 * fined behaviors. The elements of the Behavioral metamodel are shown in the 
 * following figure.
 * OCL Representation of Behavioral Constraints
 * Operations
 * The operation hasSameSignature checks if the argument has the same signature as the instance its
 * elf.
 ...
 * The operation allOperations results in a Set containing all Operations of the Classifier itself 
 * and all its inherited Operations.
 *    allOperations : Set(Operation);
 *    allOperations = self.allFeatures->select(f | f.ockIsKindOf(Operations))
 * The operation allMethods results in a Set containing all Methods of the Classifier itself and al
 * l its inherited Methods.
