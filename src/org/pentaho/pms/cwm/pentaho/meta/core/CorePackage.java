 ...
 * The ObjectModel Core metamodel contains basic metamodel classes and ass
 * ociations used by all other CWM metamodel packages, including other Obje
 * ctModel packages
 * OCL Representation of Core Constraints
 * Operations
 * The operation allFeatures results in a Set containing all Features of t
 * he Classifier itself and all its inherited Features.
 ...
 * ))
 * The operation specification yields the set of Classifiers that the curr
 * ent Classifier realizes.
 *    specification: Set(Classifier)
 *    specification = self.clientDependency->
 *    select(d | d.stereotype.name = "realization"
 *       and d.supplier.oclIsKindOf(Classifier)).supplier.oclAsType(Classi
 * fier)
 * The operation parent returns a Set containing all direct parents of a C
 * lassifier.
 *    parent : Set(Classifier);
 *    parent = self.generalization.parent
 * The operation allParents returns a Set containing all the Classifiers i
 * nherited by this Classifier (the transitive closure), excluding the Clas
 * sifier itself.
 *    allParents : Set(Classifier);
 *    allParents = self.parent->union(self.parent.allParents)
 * The operation allContents returns a Set containing all ModelElements co
 * ntained in the Classifier together with the contents inherited from its 
 * parents.
 ...
 * The operation supplier results in a Set containing all direct suppliers
 *  of the ModelElement.
 *    supplier : Set(ModelElement);
 *    supplier = self.clientDependency.supplier
 * The operation allSuppliers results in a Set containing all the ModelEle
 * ments that are suppliers of this ModelElement, including the suppliers o
 * f these Model Elements. This is the transitive closure.
 *    allSuppliers : Set(ModelElement);
 *    allSuppliers = self.supplier->union(self.supplier.allSuppliers)
 * The operation contents results in a Set containing all ModelElements co
 * ntained by the Namespace.
 ...
 * The operation allVisibleElements results in a Set containing all ModelE
 * lements visible outside of the Namespace.
 ...
 * The operation contents results in a Set containing the ModelElements ow
 * ned by or imported by the Package.
 *    contents : Set(ModelElement)
 *    contents = self.ownedElement->union(self.importedElement)
 * The operation allImportedElements results in a Set containing the Model
 * Elements imported by the Package.
 ...
 * [C-3-3] Tags associated with a model element (directly via a property l
 * ist or indirectly via a stereotype) must not clash with any meta attribu
 * tes associated with the model element.
