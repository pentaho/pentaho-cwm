 ...
 * In the metamodel, a DataType defines a special kind of Classifier in whic
 * h operations are all pure functions (i.e., they can return data values but
 *  they cannot change data values, because they have no identity). For example, 
 * an "add" operation on a number with another number as an argument yields a third
 *  number as a result; the target and argument are unchanged.
 ...
 *  as primitive values. A primitive value does not have an identity, so two occu
 * rrences of the same value cannot be differentiated. Usually, DataTypes are used 
 * for specification of the type of an attribute or parameter.
 *  
...
public interface CwmDataTypeClass extends javax.jmi.reflect.RefClass {
     ...
     * @return The created instance object.
     */
    public CwmDataType createCwmDataType(java.lang.String name, org.pentaho.pms.cwm.pentaho.meta.core.VisibilityKind visibility, boolean isAbstract);
}
